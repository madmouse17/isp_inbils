...
export default function Index({ customer, addresses }: AddressProps) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        if (editId) {
            put(route('admin.customers.addresses.update', [customer.id, editId]), { onSuccess: () => setModalOpen(false) });
        } else {
            post(route('admin.customers.addresses.store', customer.id), { onSuccess: () => setModalOpen(false) });
        }
    };

    const remove = (a: CustomerAddress) => {
        if (window.confirm(`Delete ${a.label}?`)) router.delete(route('admin.customers.addresses.destroy', [customer.id, a.id]));
    };
    ...
    return (
        <AdminLayout title="Customer Addresses">
            <div className="space-y-6">
                                    ...
                                    <TR key={a.id}>
                                        ...
                                        <TD>
                                            <div className="flex gap-2">
                                                <Button type="button" variant="ghost" size="sm" onClick={() => openEdit(a)}>Edit</Button>
                ...
                <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editId ? 'Edit Address' : 'Add Address'}>
                    <form onSubmit={submit} className="space-y-4">
                        <Input label="Label" value={data.label} onChange={(e) => setData('label', e.target.value)} error={errors.label} required />
                        <Textarea label="Address" value={data.address} onChange={(e) => setData('address', e.target.value)} error={errors.address} required rows={2} />
                        ...
                        <div className="flex gap-6">
                            <Switch label="Installation Point" checked={data.is_installation_point} onCheckedChange={(c) => setData('is_installation_point', c)} />
