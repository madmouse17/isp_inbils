...
export default function Create({ bandwidthProfiles, speedProfiles, slaTiers }: CreateProps) {
    const { data, setData, post, processing, errors } = useForm({
        code: '', name: '', bandwidth_profile_id: '', speed_profile_id: '', sla_tier_id: '',
        price_mrc: '', price_otc: '0', contract_min_months: '', description: '', is_active: true,
    });

    const submit = (e: FormEvent) => { e.preventDefault(); post(route('admin.service-packages.store')); };

    return (
        <AdminLayout title="Create Service Package">
            <form onSubmit={submit}>
                <Card>
                    <CardHeader><CardTitle>Create Service Package</CardTitle></CardHeader>
                    ...
                    <CardFooter className="justify-end gap-2">
                        <Button type="button" variant="secondary" onClick={() => router.get(route('admin.service-packages.index'))}>Cancel</Button>
