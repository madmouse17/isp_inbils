...
interface BwData {
    id: number; name: string; download_mbps: number; upload_mbps: number;
    type: string; contention_ratio: number; is_active: boolean;
}
...
export default function Edit({ bandwidthProfile }: EditProps) {
    ...
    const submit = (e: FormEvent) => { e.preventDefault(); put(route('admin.bandwidth-profiles.update', b.id)); };

    return (
        <AdminLayout title="Edit Bandwidth Profile">
            <div className="space-y-6">
                <PageHeader title="Edit Bandwidth Profile" subtitle="Fill required fields, then save." />
