...
export default function Show({ subscription }: ShowProps) {
    ...
    const status = s.status === 'active' ? 'success' : s.status === 'suspended' ? 'warning' : s.status === 'terminated' ? 'danger' : 'muted';
        ...
        <AdminLayout title={`Subscription ${s.code}`}>
            <div className="space-y-6">
                <PageHeader
                    ...
                    actions={<Button type="button" variant="secondary" onClick={() => router.get(route('admin.customers.subscriptions.index', s.customer_id))}>Back</Button>}
                />
                ...
                <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
                    <Card>
                        ...
                        <CardContent className="space-y-3 text-sm">
                            <p><span className="text-muted-foreground">Package: </span>{s.package?.name ?? `#${s.service_package_id}`}</p>
                            <p><span className="text-muted-foreground">Customer: </span>{s.customer?.name ?? `#${s.customer_id}`}</p>
                            <p><span className="text-muted-foreground">Installation Address: </span>{s.installation_address?.label ?? `#${s.installation_address_id}`}</p>
                            <p><span className="text-muted-foreground">Serving POP: </span>{s.serving_pop?.name ?? '-'}</p>
                            <p><span className="text-muted-foreground">MRC: </span>{s.mrc_amount}</p>
                            <p><span className="text-muted-foreground">OTC: </span>{s.otc_installation_fee}</p>
                            <p><span className="text-muted-foreground">Billing Day: </span>{s.billing_day}</p>
                            <p><span className="text-muted-foreground">Contract Months: </span>{s.contract_months ?? '-'}</p>
                            {s.notes && <p><span className="text-muted-foreground">Notes: </span>{s.notes}</p>}
                        </CardContent>
                    </Card>
                    <Card>
                        <CardHeader>
                        ...
                        <CardContent className="space-y-3 text-sm">
                            <p><span className="text-muted-foreground">Status: </span><StatusBadge variant={status}>{s.status}</StatusBadge></p>
                            <p><span className="text-muted-foreground">Activation Date: </span>{s.activation_date ?? '-'}</p>
                            <p><span className="text-muted-foreground">Expiration Date: </span>{s.expiration_date ?? '-'}</p>
                            <p><span className="text-muted-foreground">Next Invoice: </span>{s.next_invoice_date ?? '-'}</p>
                            <p><span className="text-muted-foreground">Terminated At: </span>{s.terminated_at ?? '-'}</p>
                            {s.terminated_reason && <p><span className="text-muted-foreground">Terminated Reason: </span>{s.terminated_reason}</p>}
                        </CardContent>
                    </Card>
                </div>
                ...
                <Modal open={actionModal !== null} onClose={() => setActionModal(null)} title={actionModal === 'suspend' ? 'Suspend Subscription' : 'Terminate Subscription'}>
                    <form onSubmit={doAction} className="space-y-4">
                        ...
                        {actionModal === 'terminate' && (
                            <div className="text-sm text-muted-foreground">ONT release will be available in Phase 3.</div>
                        )}
