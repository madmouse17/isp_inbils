...
export default function Index({ products, categories, filters, can }: IndexProps) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('admin.products.index'), { search, category_id: categoryId, is_active: isActive }, { preserveState: true });
    };

    const remove = (p: Product) => {
        if (window.confirm(`Delete ${p.name}?`)) router.delete(route('admin.products.destroy', p.id));
    };
