    ...
    return (
        <AdminLayout title="Item Finder">
            <div className="space-y-6">
                <PageHeader title="Item Finder" subtitle="Search consumable items by name or SKU." />
                <Card>
                ...
                <Card>
                    <CardContent className="space-y-4 pt-6">
                        <Table>
                            <THead><TR><TH>Product</TH><TH>SKU</TH><TH>Location</TH><TH>Path</TH><TH>Available</TH><TH>Total</TH><TH>Reserved</TH></TR></THead>
                            <TBody>
                                {results.length === 0 ? (
                                    <TR><TD colSpan={7} className="py-10 text-center text-muted-foreground">No data found.</TD></TR>
                                ) : (
                                    results.flatMap((r) => r.stocks.length === 0 ? [
                                        <TR key={`${r.id}-empty`}>
                                            <TD>{r.name}</TD>
                                            <TD className="font-mono text-sm">{r.sku}</TD>
                                            <TD colSpan={5} className="text-muted-foreground">No stock records.</TD>
                                        </TR>,
                                    ] : r.stocks.map((s, i) => (
                                        <TR key={`${r.id}-${i}`}>
                                            <TD>{r.name}</TD>
                                            ...
                                            <TD className="text-sm text-muted-foreground">{s.location_path}</TD>
