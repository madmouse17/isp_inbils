...
export default function Index({ categories, can }: IndexProps) {
    ...
    const remove = (c: Category) => {
        if (window.confirm(`Delete ${c.name}?`)) router.delete(route('admin.categories.destroy', c.id));
    };
        ...
        <AdminLayout title="Categories">
            ...
            <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Category' : 'Create Category'}>
                <form onSubmit={submitForm} className="space-y-4">
                    ...
                    <Input label="Parent ID" value={data.parent_id} onChange={(e) => setData('parent_id', e.target.value)} error={errors.parent_id} placeholder="Leave empty for root" />
                    <Textarea label="Description" value={data.description} onChange={(e) => setData('description', e.target.value)} error={errors.description} rows={2} />
