...
interface InvRow {
    ...
    total: string; paid_amount: string; issue_date: string;
...
interface IndexProps extends Record<string, unknown> {
    ...
    filters: { type?: string; status?: string; source?: string; customer_id?: string; search?: string };
    can: { create: boolean };
}
...
export default function Index({ invoices, customers, filters, can }: IndexProps) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('admin.invoices.index'), { search, type, status, customer_id: customerId }, { preserveState: true });
    };
                ...
                <Card>
                    <CardContent className="space-y-4 pt-6">
                        <form onSubmit={submit} className="flex flex-wrap gap-2">
                            <Input label="Search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Invoice number" />
                        ...
                        <Table>
                            <THead><TR><TH>Number</TH><TH>Customer</TH><TH>Type</TH><TH>Status</TH><TH>Total</TH><TH>Paid</TH><TH>Issue Date</TH><TH>Actions</TH></TR></THead>
                            <TBody>
                                {invoices.data.length === 0 ? (
                                    ...
                                    invoices.data.map((inv) => (
                                        <TR key={inv.id}>
                                            ...
                                            <TD><Badge variant="neutral">{inv.type}</Badge></TD>
                                            <TD><StatusBadge variant={statusVariant(inv.status)}>{inv.status}</StatusBadge></TD>
                                            <TD className="font-medium">{inv.total}</TD>
                                            <TD>{inv.paid_amount}</TD>
                                            <TD className="text-sm">{inv.issue_date}</TD>
                                            <TD>
                                                <Link href={route('admin.invoices.show', inv.id)} className="text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">Show</Link>
                                            </TD>
                                        </TR>
                                    ))
                                )}
