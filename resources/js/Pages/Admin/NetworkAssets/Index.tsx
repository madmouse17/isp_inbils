...
interface AssetRow {
    ...
    status: string; location?: { name: string } | null;
}
...
export default function Index({ assets, locations, filters, can }: IndexProps) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('admin.network-assets.index'), { search, asset_type: assetType, status, location_id: locationId }, { preserveState: true });
    };

    const remove = (a: AssetRow) => {
        if (window.confirm(`Delete ${a.name}?`)) router.delete(route('admin.network-assets.destroy', a.id));
    };
            ...
            <div className="space-y-6">
                ...
                <Card>
                    <CardContent className="space-y-4 pt-6">
                            ...
                            <TBody>
                                {assets.data.length === 0 ? (
                                    ...
                                    <TR key={a.id}>
                                        ...
                                        <TD>{a.name}</TD>
                                        <TD><Badge variant="neutral">{a.asset_type}</Badge></TD>
                                        <TD className="font-mono text-sm">{a.serial_number ?? '-'}</TD>
                                        <TD><StatusBadge variant={a.status === 'available' ? 'success' : a.status === 'installed' ? 'info' : a.status === 'maintenance' ? 'warning' : a.status === 'damaged' ? 'danger' : 'muted'}>{a.status}</StatusBadge></TD>
                                        <TD>{a.location?.name ?? '-'}</TD>
                                        <TD>
                                            <div className="flex gap-2">
                                                <Link href={route('admin.network-assets.show', a.id)} className="text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">Show</Link>
                                                <Link href={route('admin.network-assets.edit', a.id)} className="text-sm font-medium text-brand-600 hover:text-brand-700 dark:text-brand-400">Edit</Link>
                                                <Button type="button" variant="ghost" size="sm" onClick={() => remove(a)}>Delete</Button>
                                            </div>
                                        </TD>
                                    </TR>
                                ))}
                        ...
                        <Pagination currentPage={assets.current_page} lastPage={assets.last_page} onPageChange={(page) => router.get(route('admin.network-assets.index'), { page })} />
                    </CardContent>
                </Card>
            </div>
        </AdminLayout>
