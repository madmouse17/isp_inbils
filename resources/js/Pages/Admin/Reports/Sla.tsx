...
interface SlaData extends Record<string, unknown> {
    total_resolved?: number; sla_compliant?: number; sla_rate?: number; breach_count?: number;
    avg_resolution_minutes?: number | null; avg_frt_minutes?: number | null;
...
export default function Sla({ data, filters }: Props) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('admin.reports.sla'), { date_from: dateFrom, date_to: dateTo }, { preserveState: true });
    };
                ...
                {data && (
                    <div className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-4">
                            <Card><CardContent><p className="text-sm text-surface-500">Total Resolved</p><p className="text-2xl font-bold">{data.total_resolved ?? 0}</p></CardContent></Card>
                            <Card><CardContent><p className="text-sm text-surface-500">SLA Rate</p><p className="text-2xl font-bold">{data.sla_rate ?? 0}%</p></CardContent></Card>
                            <Card><CardContent><p className="text-sm text-surface-500">Breaches</p><p className="text-2xl font-bold">{data.breach_count ?? 0}</p></CardContent></Card>
