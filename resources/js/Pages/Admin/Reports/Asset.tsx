...
export default function Asset({ data, filters }: Props) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        router.get(route('admin.reports.asset'), { asset_type: assetType, date_from: dateFrom, date_to: dateTo }, { preserveState: true });
    };
                ...
                {data && (
                    <div className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-3">
                            <Card><CardContent><p className="text-sm text-surface-500">Installations</p><p className="text-2xl font-bold">{data.installation_count ?? 0}</p></CardContent></Card>
                            <Card><CardContent><p className="text-sm text-surface-500">Customer-Linked</p><p className="text-2xl font-bold">{data.customer_linked_count ?? 0}</p></CardContent></Card>
                            <Card><CardContent><p className="text-sm text-surface-500">Retired</p><p className="text-2xl font-bold">{data.retired_count ?? 0}</p></CardContent></Card>
                        </div>
                        <Card>
                            <CardContent>
